...
class LeftLegTeleopNode : public rclcpp::Node
{
public:
  LeftLegTeleopNode()
  : Node("left_leg_teleop_node")
  {
    joy_topic_ = declare_parameter<std::string>("joy_topic", "joy");
    cmd_topic_ = declare_parameter<std::string>("command_topic", "/hros5/left_leg/target_angles_deg");
    echo_request_topic_ = declare_parameter<std::string>("echo_request_topic", "/hros5/left_leg/echo_positions");
    ...
    joy_sub_ = create_subscription<sensor_msgs::msg::Joy>(
      joy_topic_, 10, std::bind(&LeftLegTeleopNode::joy_callback, this, std::placeholders::_1));

    timer_ = create_wall_timer(
      std::chrono::duration_cast<std::chrono::nanoseconds>(
        std::chrono::duration<double>(1.0 / publish_rate_hz_)),
      std::bind(&LeftLegTeleopNode::publish_command, this));

    RCLCPP_INFO(get_logger(), "LeftLegTeleopNode started.");
  }
...
private:
  ...
  void apply_preset(const std::array<double, 6>& preset, const char* label)
  {
    ...
    RCLCPP_INFO_THROTTLE(
      ...
      "Applied left leg preset: %s [%.1f, %.1f, %.1f, %.1f, %.1f, %.1f]",
      label,
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<LeftLegTeleopNode>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
