...
public:
  DxlNode() : rclcpp::Node("left_leg_dxl_node") {
    ...
    declare_parameter<std::string>("config_file", "config/joints/left_leg.yaml");
    declare_parameter<std::string>("command_topic", "/hros5/left_leg/target_angles_deg");
    declare_parameter<std::string>("echo_request_topic", "/hros5/left_leg/echo_positions");
    ...
    const std::array<std::pair<std::string, std::string>, 6> joint_names{{
      {"LHipYaw",      "lhip_yaw"},
      {"LHipRoll",     "lhip_roll"},
      {"LHipPitch",    "lhip_pitch"},
      {"LKneePitch",   "lknee_pitch"},
      {"LAnklePitch",  "lankle_pitch"},
      {"LAnkleRoll",   "lankle_roll"},
    }};
...
private:
  void cb(const std_msgs::msg::Float32MultiArray::SharedPtr msg) {
    ...
    for (size_t i = 0; i < joints_.size(); ++i) {
      ...
      double deg = clip(msg->data[i] + get_parameter(param_name(joint.prefix, "offset_deg")).as_double(),
                        min_deg, max_deg);
      joint.target_deg = invert ? -deg : deg;
    }
  }
  ...
  void update() {
    ...
    std::vector<int> goal_ticks(joints_.size(), 0);
    bool any_change = false;

    for (size_t i = 0; i < joints_.size(); ++i) {
      auto& joint = joints_[i];

      double step = clip((joint.target_deg - joint.current_deg) * kp, -max_step, max_step);
      joint.current_deg += step;

      double deg = clip(joint.current_deg, get_parameter(param_name(joint.prefix, "min_deg")).as_double(),
                        get_parameter(param_name(joint.prefix, "max_deg")).as_double());

      int ticks = clamp_ticks(static_cast<int>((deg + 180.0) * TICKS_PER_DEG), joint.info);
      goal_ticks[i] = ticks;

      if (std::abs(step) > 1e-3) {
        any_change = true;
      }
    }
    ...
    for (size_t i = 0; i < joints_.size(); ++i) {
      write4(joints_[i].id, ADDR_GOAL_POSITION, static_cast<uint32_t>(goal_ticks[i]));
    }
  }
  ...
  void write1(uint8_t id, uint16_t addr, uint8_t value) {
    dxl_with_retry(bus_config_, port_, get_logger(), "write1", [&](uint8_t& err){
      return packet_->write1ByteTxRx(port_, id, addr, value, &err);
    });
  }

  void write4(uint8_t id, uint16_t addr, uint32_t value) {
    dxl_with_retry(bus_config_, port_, get_logger(), "write4", [&](uint8_t& err){
      return packet_->write4ByteTxRx(port_, id, addr, value, &err);
    });
  }
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<DxlNode>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
